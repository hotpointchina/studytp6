<?php /*a:1:{s:85:"D:\ProgramFiles\Development\php\wamp\wamp64\www\studytp6\view\certificates\index.html";i:1694014627;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
    ...
    <div class="dept_all">
      ...
      <div class="flex flex-col w-full px-20px">
        ...
        <div class="base-table">
          ...
          <el-table :data="ztableData" stripe style="width: 100%">
          ...
          <div class="py-5 flex justify-end">
            <!-- <el-config-provider :locale="zhCn"> -->
              <el-pagination v-model:currentPage="zmockStore.currentPage" v-model:page-size="zmockStore.pageSize"
                :page-sizes="zmockStore.pageSizeList" :small="true" :disabled="false"
                layout="total, sizes, prev, pager, next, jumper" :total="zmockStore.total"
        ...
        <el-dialog v-model="tableData.detailShow" title="详情" width="60%" top="15vh" :draggable="true">
          <el-descriptions class="margin-top" :column="3" :size="'default'" :style="''" :border="true">
            <el-descriptions-item v-for="des in tableData.descriptions" :key="des.prop" :label="des.label">
              ...
              <el-tag class="ml-2" :type="tableData.detailInfo[des.prop as string] =='启用'?'success':'danger'">{{ tableData.detailInfo[des.prop as string] }}</el-tag>
            </template>
            <template v-else>
...
<script>
  ...
  const option = {
    setup() {
      const { Plus,Close } = ElementPlusIconsVue;
      // vant.showToast('提示');
      ...
      // 批量删除
      const deleteChoose = () => {
        // zmockStore.deleteChoose( multipleSelection.value );
        let template = JSON.parse(JSON.stringify(zmockStore.list));
        multipleSelection.value.forEach(obj => {
          template = template.filter(rowItem => rowItem.idNumber != obj.idNumber);
        });
        zmockStore.list.length = 0;
        template.forEach(it => zmockStore.list.push(it));
        template = null;
      };
